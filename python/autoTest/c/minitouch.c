// 如果要构建自行，拉取仓库代码，替换该文件,就不在工程内加入该仓库
...
static int start_inter_server(char* socket_path)
{
  int fd = socket(AF_INET, SOCK_STREAM, 0);
    if (fd < 0)
  {
    perror("creating internet socket");
    return fd;
  }

  char* char_buff = NULL;
  char* ip = strtok_r(socket_path, ":", &char_buff);
  char* port_str = strtok_r(NULL, ":", &char_buff);
  ...
  {
    perror("binding internet socket");
    close(fd);
    return -1;
  }
  listen(fd, 20);
...
int main(int argc, char* argv[])
{
  ...
  int opt;
  while ((opt = getopt(argc, argv, "s:d:n:vif:h")) != -1) {
    switch (opt) {
      case 'd':
        device = optarg;
        break;
  ...
  socklen_t in_client_addr_length = sizeof(in_client_addr);

  int server_fd = -1;
  if (use_inter == 1){
    server_fd = start_inter_server(ip_path);
  }
  else{
    server_fd = start_server(sockname);
  }
  ...
  while (1)
  {
    ...
    if(use_inter == 1)
    {
      client_fd = accept(server_fd, (struct sockaddr *) &in_client_addr, &in_client_addr_length);
    }
    else
    {
